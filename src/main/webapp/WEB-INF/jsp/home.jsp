...
<div class="card">
 <a href="tvinfo?id=${entry.getTvId()}&name=${entry.getTvName()}&brand=${entry.getTvBrand()}&price=${entry.getTvPrice()}&des=${entry.getTvDescription()}"> <img alt="Bravia TV" src="${entry.getTvPath()}"  > </a>
  <h1>${entry.getTvName()}</h1>
  <p class="price">Rs. ${entry.getTvPrice()}</p>
  <p>${entry.getTvDescription()}</p>
  <p><a href="login"> <button>Add to Cart</button> </a></p>
</div>
